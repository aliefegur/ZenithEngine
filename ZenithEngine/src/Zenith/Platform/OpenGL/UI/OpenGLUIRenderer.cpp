...
namespace Zenith
{
	...
	OpenGLUIRenderer::OpenGLUIRenderer(Graphics* gfx, Shader* textShader, Shader* imageShader)
		:
		UIRenderer(gfx, textShader, imageShader)
	{
	}
	
	void OpenGLUIRenderer::DrawImage(Texture2D* _img, const glm::vec2& position, const glm::vec2& scale, float rotation, Anchor anchor, Anchor pivot, const glm::vec4& color)
	{
		UIRenderer::DrawImage(_img, position, scale, rotation, anchor, pivot, color);

		auto img = reinterpret_cast<OpenGLTexture2D*>(_img);
	...
	void OpenGLUIRenderer::DrawChar(Font* _fontFamily, char chr, const glm::vec2& position, float fontSize, const glm::vec4& color)
	{
		UIRenderer::DrawChar(_fontFamily, chr, position, fontSize, color);

		auto fontFamily = reinterpret_cast<OpenGLFont*>(_fontFamily);
		OpenGLFont::Character c = fontFamily->m_Characters[chr];
		uint32_t cTextureID = fontFamily->m_TextureIDs[chr];

		// Is the texture has used in the current batch?
		float texIndex = -1.0f;
		for (uint32_t i = 0; i < m_TextTextureSlotIndex; i++)
		{
			if (m_TextTextureSlots[i] == cTextureID)
			{
				texIndex = static_cast<float>(i);
				break;
			}
		...
		if (texIndex == -1.0f)
		{
			...
			m_TextTextureSlots[m_TextTextureSlotIndex] = cTextureID;
	...
	void OpenGLUIRenderer::InitializeTextRenderer(Shader* textShader)
	{
		UIRenderer::InitializeTextRenderer(textShader);
	...
	void OpenGLUIRenderer::InitializeImageRenderer(Shader* imageShader)
	{
		UIRenderer::InitializeImageRenderer(imageShader);
	...
	void OpenGLUIRenderer::DestroyTextRenderer()
	{
		UIRenderer::DestroyTextRenderer();
	...
	void OpenGLUIRenderer::DestroyImageRenderer()
	{
		UIRenderer::DestroyImageRenderer();
	...
	void OpenGLUIRenderer::EndTextRenderer()
	{
	...
	void OpenGLUIRenderer::EndImageRenderer()
	{
		// Load vertex data to the GPU
